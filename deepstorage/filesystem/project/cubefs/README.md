...
CubeFS 是 CNCF 毕业项目（原 ChubaoFS，京东开源），是一个**自研全栈**的分布式文件系统：
自研 Master / MetaNode / DataNode 三组件、卷 + 分区模型、副本与 EC 双栈、原生小文件打包。
对于打算自研元数据与数据服务的读者，它的取舍比 JuiceFS 更具直接参考价值——包括它踩到的坑。
...
| [cubefs-analysis.md](cubefs-analysis.md) | 设计评估：可借鉴的机制、需规避的设计、与 JuiceFS 的分歧点 |
