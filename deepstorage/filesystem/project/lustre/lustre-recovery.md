...
**对通用场景，用 Raft 之类的多副本共识承担持久性是更稳妥的选择**。
但 transno + 客户端重放这个思路在一个地方仍有价值：**客户端的写入重试语义**。
如果客户端攒批提交 extent 索引，元数据服务主切换可能导致提交丢失，
客户端应该能重放——这需要客户端侧保留"已发出但未确认已提交"的索引，
与 Lustre 的机制同构。
...
解决同一问题。即使元数据服务用 Raft 复制，也需要等价机制——
...
用 Raft 做元数据复制的系统不存在"等客户端重放"的恢复窗口，所以 VBR 不直接适用。

但**"版本号 = 最后修改的事务序号 + 前置版本校验"这个模式**在另一个场景直接可用：
**客户端攒批提交 extent 索引时的冲突检测**。

设想：客户端 A 和客户端 B 同时写同一个文件的重叠区间，各自攒批后提交。
如果索引提交只是无条件覆盖，后到的赢——但"后到"由网络决定，不是由写入顺序决定。

带上前置版本可以让元数据服务检测到冲突：**提交时校验 inode 的
extent 索引版本是否仍是我读到的那个**，不是则拒绝并让客户端重新拉取后重试。
...
### 设计启示：客户端写缓冲必须回答"空间用尽"问题 ★★★

这是本次调研中**最容易被忽略、但对新系统设计最实际的一个点**。

任何在客户端做写缓冲、攒批后再提交的系统（例如"累计到一定数据量或
Sync()/Close() 时批量提交"）都存在同样的问题：**`write()` 返回后数据还在
客户端内存里，真正落盘时如果目标存储单元满了/集群满了怎么办？**

可选方案：

1. **Grant 式预留**：客户端从管理服务或数据节点领取空间额度，
   额度不足时阻塞或降级为同写。语义最强，实现最复杂。
2. **写前分配**：`write()` 时就向数据节点预留空间（不写数据），
   Sync 时才真正写入。折中方案。
3. **接受语义弱化**：明确文档化"只有 Sync/Close 返回成功才保证持久"，
   把问题推给应用。这是 JuiceFS/CubeFS 的实际做法。

**至少要显式做出选择并写进设计文档**，而不是默认第 3 种。
...
**设计启示**：在数据记录头部保存反向指针（所属文件 + 文件内偏移），
是让孤儿清理从 O(全局) 降到 O(局部) 的前提。更进一步，
若回收能在单个存储单元内闭环（例如卷内 compaction 时按反向指针核对是否仍被引用），
就不需要独立的检查工具。
...
对已有中心化成员管理与心跳的系统，主动通知的条件是现成的——
分片主切换、存储单元密封、数据节点下线都应该走主动通知路径，
而不是让客户端靠 RPC 失败去发现。
