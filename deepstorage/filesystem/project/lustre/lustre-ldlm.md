...
精确划分。LightStore 若引入任何形式的分布式锁，这个矩阵值得作为起点，
而不是从读写锁开始演进。
...
对 LightStore：`stat` 一个正在被写的文件，SDK 侧写缓冲里的数据不在元数据中，
返回的长度必然偏小。如果目标场景包含"一边写一边有人监控进度"
...
这个模式在 LightStore 里有多处适用：卷分配、租约、索引提交批次大小。
...
**对 LightStore 的启示**：如果引入任何客户端元数据缓存的失效机制，
不要用 inode 粒度。至少要区分：
