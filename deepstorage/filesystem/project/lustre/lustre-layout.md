...
**这是与 JuiceFS/CubeFS/LightStore 最根本的模型差异**：

| | Lustre | CubeFS / LightStore |
|---|--------|-------------------|
...
且 HPC 场景文件数少、单文件大。**LightStore 的万亿小文件场景不适用这个模型**，
但它提醒了一件事：**当写入是纯顺序大块时，公式化索引能把元数据量压到近乎为零**。
LightStore 的 extent 索引可以借鉴——见 [分析文档](lustre-analysis.md) §2.1。
...
LightStore 的 volume 选择策略（小文件打包 vs 大文件独立 record）
本质是同一个问题的另一种解法——用 record 大小动态决定，
比 PFL 的预设区间更自适应，但失去了"用户可显式指定"的能力。
...
（PFL → SEL → OST pool → 空间均衡策略）。LightStore 的"写时选卷"模型天然回避了这类问题。
...
对比 CubeFS 的 write-all 和 LightStore 的链式复制——**它们提供的是真正的
写入时冗余，FLR 不是**。这是 Lustre 的历史包袱：它的可靠性模型建立在
"后端 RAID 不会丢数据"之上，FLR 是事后补的用户态特性。

**LightStore 不应该走 FLR 路线**，卷级同步复制是更正确的选择。
...
对照：LightStore 的 Haystack 打包把小文件放在 DataServer 的卷里，
不占用 MetaServer 资源，只是多一次 IO。**在万亿小文件的目标下，
LightStore 的选择明显更可扩展**——DoM 方案下 MDT 容量会成为瓶颈。
...
LightStore 的 Range 分裂/合并/迁移面临同样问题。这个"水位分界 + 读路径双向路由"
的手法比"迁移期间锁住整个 Range"要好得多，值得参考。
...
对照 LightStore 的"Range 随容量与负载自动分裂、合并、迁移"——
**自动化是 LightStore 相对 Lustre 的明确优势**，也是必须的：
万亿文件规模下不可能靠人工运维分片。但要注意 Lustre 不做自动分裂
可能有它的道理：自动分裂在高负载下触发，会进一步加剧负载。
LightStore 的分裂策略需要有**滞回与限流**，避免抖动。
...
这是一套完整的"布局版本 fencing"，与 CubeFS 的 `vuid.epoch`、
LightStore 的 `vol_epoch` 同源，但作用在文件布局层而非卷层。
