...
CubeFS 是一个自研全栈的分布式文件系统，已经在生产中跑了多年。
对于要设计新的分布式存储系统的读者，它做对的地方是验证，它撞到的墙是预警。
本篇按"可借鉴的设计 / 需规避的设计 / 分歧点 / 结论"整理。

## 1. 设计要点总表

| 维度 | CubeFS 的选择 | 评价 |
|------|--------------|------|
| 管控 | Master（Raft） | 标准做法；但缓存了 per-extent 状态（§3.3） |
| 元数据 | MetaNode，**全内存 BTree** + Raft + 5 分钟快照 | 规模上限 = 内存容量（§3.1） |
| 元数据分片 | **inode ID 区间** | 目录局部性丢失，被迫做分布式事务（§3.2） |
| 数据容器 | Data Partition 120 GiB，3 副本 | 放置/复制/修复的单元，Master 只维护 O(DP 数) 状态 |
| 数据寻址 | ExtentKey{DP, ExtentID, ExtentOffset, Size, CRC} | `(容器, 容器内位置, 长度)` 三元组 + 一层 extent 间接 |
| 小文件 | Tiny Extent（64 个/DP，共享追加） | Haystack 式打包，独占写者避免偏移竞争 |
| 覆盖写 | 写时原地替换 extent 列表 | 读快、无 compaction；代价在 Raft apply 内（§5） |
| 副本复制 | 星型主副本转发，**write-all** | 3 副本下合理，失败即换址 |
| EC | BlobStore 独立子系统，**quorum 写** | 分片多时必须 quorum（§2.1） |
| 失败处理 | 换 extent/DP 重放（seal-and-new） | append-only 的红利 |
| 空间回收 | tiny 打洞 / normal 删文件 / EC compact | 两种回收策略并存，各有适用场景（§2.2） |
| 后台任务 | 服务端驱动 | 不依赖客户端在线 |

**结论：CubeFS 的数据层（副本、EC、小文件打包、空间回收、失败处理）是一份经过生产验证的
参考实现；它的元数据层（全内存、按 inode ID 分片）则是需要规避的前车之鉴。**
...
对于同时有副本路径和 EC 路径的系统，写入确认策略应该分开明确：

- 副本卷：write-all，3 副本下代价可控，失败即换址重写
- EC 卷：quorum 确认，剩余分片后台补齐，缺失分片可由 EC 重建

quorum 下界可以参考 CubeFS 的约束（`blobstore/common/codemode/codemode.go:165`）：

```
(N + M) / AZCount + N  ≤  PutQuorum  ≤  N + M
```

以 RS(12,4) 单 AZ 代入：`16/1 + 12 = 28 > 16`，说明**单 AZ 下这条不等式无解**——
CubeFS 的 EC12P4 配的是 `PutQuorum = 15`，即只允许 1 个分片后台补齐。
这条不等式本身是"容忍整 AZ 故障"的约束，单 AZ 场景不适用，
但它提醒了一件事：**quorum 取值必须保证已确认数据在预期故障域失效后仍可重建**，
需要按自身的故障域模型推导这个下界，不能凭感觉取 N+1。
...
**对 append-only 卷设计的价值**：这类系统通常把卷的回收路径设计成
"密封 → compaction 搬迁存活记录 → 丢弃旧卷"。如果记录存在本地文件里，
...
1. **粒度是文件系统块**（通常 4 KiB）。小于一块的删除回收不了空间。
   小文件密集场景下这个浪费不可忽略。
2. **打洞不改变存活记录的地址**。寻址结构里直接含卷内 `offset` 的设计，
   打洞只能回收空间，**不能提升卷的空间连续性**。想真正压缩卷、
...
推广为通用规则：客户端在小文件打包路径上写失败时，
除了换一个数据容器，还应考虑降级到独立记录路径，避免在同一个瓶颈上反复撞。
...
文件系统的 extent 索引通常是 `(inode, file_offset) → 位置` 逐条记录的。
大文件顺序写时，多条记录大概率在同一个数据容器内连续，
可以压缩成 `(container_id, start_offset, record_count, record_size)`。
**对 TB 级大文件，元数据条数可降一到两个数量级**，
直接减轻元数据服务的存储与 Raft 日志压力。

代价是覆盖写时要分裂游程，实现复杂度上升。建议：
**只在批量提交索引时做游程合并**，覆盖写路径保持逐条，
用后台整理把碎片重新合并成游程。
...
epoch fencing 本身是分布式存储的标准机制；CubeFS 值得借鉴的细节是
**把 epoch 编进寻址标识本身**，而不是作为旁路检查——
...
两个改动成本都极低。自定义的记录格式可以在头部加同样的标志位，
稀疏文件和预分配场景直接受益。
...
更可扩展的方向是"分片体积有界 + 数据落盘（LSM 类引擎）+ 增量刷盘"。
**这是本次调研最重要的一条反面教训：面向超大规模的元数据服务不能全内存。**
...
替代方案是 key 范围分片 + "子 inode 尽量与父目录同分片"，可以直接规避这三条。

但采用替代方案时要注意两个退化场景，必须被设计和测试覆盖：

- **超大目录跨分片**：十亿级条目的目录不可能只落在一个分片里，
  此时 dentry 与父目录 inode 就分离了
- **inode 段耗尽**："尽量同分片"意味着一定有退化路径

跨分片事务的实现不能省。CubeFS 的 `transaction.go` 虽然是被逼出来的，
...
**这违背了中心管控组件的基本原则**："只保存 O(集群规模) 的状态，
不保存任何 per-file / per-record 信息"。

设计时即便明确禁止了这一点，也要警惕它以隐蔽形式回来：
"为了做一致性检查/修复调度，中心组件临时拉一份记录列表"——
这类需求会不断出现，必须坚持**把检查下推到数据容器的主副本**，
中心组件只接收聚合后的结论。
...
如果新系统一开始就规划了副本 → EC 的转码或冷热迁移，**建议从第一天起就把
"数据当前位置"设计成统一的位置列表 + 一个存储级别标记，而不是为每种迁移状态
加一组字段。** 迁移中的临时状态应该放在独立的迁移任务表里，不要污染 inode 结构。

## 4. 经过生产验证的选择

这些是 CubeFS 在生产中验证过的选择（其中 append-only 与客户端攒批提交也被 JuiceFS 采用），
设计新系统时可以放心沿用：

1. **数据容器约 100 GiB 量级**（CubeFS DP 120 GiB）
2. **append-only + 异地覆写**，修复退化成长度对齐
3. **客户端攒批提交索引**（handler 关闭时提交 ExtentKey）
4. **失败即换址重写**（seal-and-new）
5. **EC 参数 RS(12,4)，1.33x**（EC12P4）
6. **小文件必须打包**，不能一文件一对象

## 5. 一个必须站队的分歧：覆盖写的展开时机

这是 JuiceFS 与 CubeFS 的正面分歧，设计 extent 索引时必须明确选择：
...
选择 CubeFS 路线（写时按区间替换旧项）的连带后果：

- **好处**：读路径干净、不需要 compaction 触发策略、GC 是定向的
  （覆盖时就知道哪些记录死了，直接进删除日志）——与
  "GC 由删除日志订阅驱动"的后台任务模型天然契合
- **代价**：区间替换发生在元数据服务的 Raft apply 内，
  单文件 extent 数很大时（随机写密集）是 O(n) 数组搬移，会拖慢整个分片的 apply

**建议**：采用 CubeFS 路线，但补一条保护——
...
1. 数据容器 ~120 GiB、append-only、seal-and-new、EC RS(12,4)（§4）
2. 小文件打包、后台任务服务端驱动
3. 定向删除：extent 归属明确，不需要全局扫描找孤儿
...
4. 跨分片事务路径（超大目录、inode 段耗尽的退化场景）（§3.2）
